...
show_data_menu() {
    ...
    ui_print " "
    ui_print "📁 选择数据目录"
    ui_print "1、data/adb/alist"
    ui_print "2、Android/alist"
    ui_print "━━━━━━━━━━━━━━━━━━━━━━"
    ui_print "音量+ 确认  |  音量- 切换"
    ui_print "━━━━━━━━━━━━━━━━━━━━━━"
    ui_print "👉 当前选择：选项 $current"
...
if echo "$ARCH" | grep -q "arm64"; then
    ...
    if [ -f "$MODPATH/alist-arm64" ]; then
        mv "$MODPATH/alist-arm64" "$BINARY_PATH/$BINARY_NAME"
        rm -f "$MODPATH/alist-arm"
    else
        abort "❌ 错误：未找到 ARM64 版本文件"
    fi
else
    ui_print "📦 安装 ARM 版本..."
    if [ -f "$MODPATH/alist-arm" ]; then
        mv "$MODPATH/alist-arm" "$BINARY_PATH/$BINARY_NAME"
        rm -f "$MODPATH/alist-arm64"
    else
